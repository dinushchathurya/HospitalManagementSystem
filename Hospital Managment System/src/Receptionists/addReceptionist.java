...
public class addReceptionist extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        rStatusField.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Single", "Married" }));
...
//        rPassField.setText("");
    ...
    //View next id when form load
    private void nextId() {
        connection = Connector.ConnectDb();
        if (connection != null) {
            String sql = "select `count`+1 as next_id from receptionist order by `count` desc limit 1;";
            try {
                prp = connection.prepareStatement(sql);
                rs = prp.executeQuery();
                int nextId = 0;
                ...
                rIdField.setText(String.valueOf(nextId));
